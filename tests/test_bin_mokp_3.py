...
class BinMokp3():
    def __init__(self, array_combination):
        ...
        self.directions = ["max", "max", "max"] if array_combination[8] == "max" else ["min", "min", "min"]
    ...
    def define_value_vector_knasapck(self, number):
        weights = []
        profits = []
        with open("instances/bin_mokp_" + str(self.num_of_variables) + "_" + str(number) + "_.txt", "rt") as f:
    ...
    def get_profit(self, individual, capacity, weights, profits):
        ...
        return profit if weight <= capacity else 0
    ...
    def f3(self, individual):
        capacity, weights, profits = self.define_value_vector_knasapck(number=2)
        return self.get_profit(individual, capacity, weights, profits)
    ...
    def do(self):
        ...
        # if self.algorithms!="Moead":
        #     print("Testing combination: " + str(
        #         [self.num_of_variables, self.spea2_archive_size, self.tournament_size, self.crossover_prob, self.num_of_individuals, self.mutation_rate]))
        # else:
        #     print("Testing combination: " + str(
        #         [self.num_of_variables, self.num_of_individuals, self.moead_neighborhood_size, self.mutation_rate]))

        knapClass = self.set_knap()

        problem = Problem(num_of_variables=self.num_of_variables,
                          num_of_individuals=self.num_of_individuals,
                          objectives=[self.f1, self.f2, self.f3],
                          variables_range=[0, 1],
        ...
        #     algorithm = self.algorithms
        print("Starting iteration = " + str(self.iteration))
        random.seed()
        if algorithm=="Nsga2":
            iteration = Nsga2(problem=problem,
                              num_of_generations=self.generations,
                              num_of_tour_particips=self.tournament_size,
                              tournament_prob=self.crossover_prob,
                              mutation_param=self.mutation_rate)
            obj_func, tempoTotal, memoria, nondominated = iteration.evolve()
        elif algorithm=="Spea2":
            iteration = Spea2(problem=problem,
                              num_of_tour_particips=self.tournament_size,
                              tournament_prob=self.crossover_prob,
                              mutation_rate=self.mutation_rate,
                              num_of_generations=self.generations,
                              archive_size=self.spea2_archive_size)
            ...
            iteration = Smsemoa(problem=problem,
                              num_of_generations=self.generations,
                              num_of_tour_particips=self.tournament_size,
                              tournament_prob=self.crossover_prob,
                              mutation_rate=self.mutation_rate)
            ...
            iteration = Msea(problem=problem,
                             num_of_tour_particips=self.tournament_size,
                             tournament_prob=self.crossover_prob,
                             mutation_param=self.mutation_rate,
                             num_of_generations=self.generations)
        ...
        for individual in nondominated:
            ...
            list_z.append(individual.objectives[2])

        print("x_" + algorithm + "=" + str(list_x))
        print("y_" + algorithm + "=" + str(list_y))
        print("z_" + algorithm + "=" + str(list_z))

        directory = "tests/results/" + algorithm + "/" + "bin_mokp_3/"

        Path(directory).mkdir(parents=True, exist_ok=True)
        file_results = directory + algorithm + "_knap_3_" + str(self.num_of_variables) + "_iterations.txt"
        file_solutions = directory + algorithm + "_knap_3_" + str(self.num_of_variables) + "_solutions.txt"
        file_statistics = directory + algorithm + "_knap_3_" + str(self.num_of_variables) + "_statistics.txt"
        file_obj_gen = directory + algorithm + "_knap_3_" + str(self.num_of_variables) + "_igd_gen.txt"
        ...
        with open(file_results, "a+") as f:
            ...
            f.write("z_" + algorithm + "=" + str(list_z) + "\n")
